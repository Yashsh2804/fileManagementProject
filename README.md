...
📁 Project Structure
FileManagementSystem/ │ ├── src/ # All C++ source files │ └── file_management_tool.cpp │ ├── include/ # (Optional) header files if you split code later │ └── file_manager.h │ ├── bin/ # Compiled executable goes here │ └── file_tool # (file_tool.exe on Windows) │ ├── data/ # Text files created/read/appended by your program │ └── data.txt │ ├── docs/ # Documentation for internship submission │ └── Project_Report.pdf │ └── README.md # Instructions for compiling & running
...
Examle Usage: ===== FILE MANAGEMENT TOOL ===== Enter the file name (e.g., data.txt): data/data.txt
